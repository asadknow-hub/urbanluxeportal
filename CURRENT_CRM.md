...
**Status:** this is the **live** CRM as implemented in the UrbanLuxe Portal codebase (August 2026): one person from first contact, internal inventory, bookable viewings, a week calendar, requirement matching, desks, and **mandatory Postgres RLS** (R0/R1 ownable reads shipped).
...
**Agent visibility (Postgres + UI):**
- Leads board/list/dashboard: `assigned_to = me OR assigned_to IS NULL`, then RLS keeps only the agent’s desk (or the house unassigned pool if they have no desk).
- Deals: `assigned_to = me` only (`crm_can_read_deal`).
- Viewings calendar and dashboard today-strip query `lead_viewings` with the **user JWT** (`crm_can_read_viewing`).
...
Agents see **their** numbers (own + unassigned pool, gated by RLS); admin/manager/reception/accountant see agency-wide (accountant has `dashboard_full`). Today’s viewings load on the user JWT, not the service role.
