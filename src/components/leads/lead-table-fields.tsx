import { leadTableFieldGroups } from "@/lib/lead-table-fields";

export function LeadTableFieldsColumn() {
  const groups = leadTableFieldGroups();
    ...
    <div className="rounded-xl border border-border bg-card">
      <div className="border-b border-border px-4 py-3">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Every column stored on a lead, in the same shape as the database table.
        </p>
      </div>
          ...
          <div key={group.name} className="px-4 py-3">
            ...
            <ul className="flex flex-col gap-1">
              {group.fields.map((field) => (
                <li
                  key={field.key}
                  className="flex items-baseline justify-between gap-3 rounded-md px-1 py-1.5"
                >
