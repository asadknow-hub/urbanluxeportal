...
import { CustomerOwnerSection } from "@/components/customers/customer-owner-section";
...
export default async function CustomerDetailPage({
  ...
  const canManage = canManageCrm(user.role);
  const allDocCategories = leadDocChecklistCategories((docCategoryRows ?? []) as LeadFieldOption[]);
  const docCategories = allDocCategories.filter((cat) => cat.scope !== "property");
  const propertyDocValues = new Set(
    allDocCategories.filter((cat) => cat.scope === "property").map((cat) => cat.value)
  );
  ...
  const personDocuments = mergedDocuments.filter((doc) => {
    const value = doc.category;
    if (propertyDocValues.has(value)) return false;
    if (defaultDocScope(value) === "property") return false;
    return true;
  });
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1460px] flex-col gap-[18px]">
      ...
      <PersonDocumentsKycSection
        ...
        canEdit={canEdit}
        sourcesHint="Person documents from this customer, the originating lead, and linked deals. Property unit files live on Inventory."
        overview={
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1fr_360px]">
            <div className="space-y-4">
              <div className="overflow-hidden rounded-[14px] border border-emerald-200/80 bg-emerald-50/45">
                <div className="h-0.5 bg-emerald-600" />
                <div className="border-b border-emerald-200/70 bg-emerald-100/55 px-5 py-4">
                  <h2 className="text-sm font-semibold text-emerald-950">
                    Transactions
                    <span className="ml-2 font-medium text-emerald-800/70">
                      ({properties?.length ?? 0})
                    </span>
                  </h2>
                  <p className="mt-0.5 text-xs text-emerald-900/65">Closed deals linked to this client</p>
                </div>
                {(properties ?? []).length === 0 ? (
                  <p className="px-5 py-6 text-sm text-emerald-900/60">No closed transactions yet.</p>
                ) : (
                  <div className="divide-y divide-emerald-200/60">
                    {(properties ?? []).map((prop) => {
                      const agentLabel = prop.agent?.full_name ?? prop.agent_name;
                      const paymentLines = paymentSnapshotLines(
                        parsePaymentSnapshot(prop.payment_snapshot)
                      );
                      const dealType = prop.deal_type.replace(/_/g, " ");
                      const propertyHref = prop.property_id
                        ? `/inventory/${prop.property_id}`
                        : `/company-properties/${prop.id}`;
                      return (
                        <div key={prop.id} className="bg-white/40 px-5 py-4">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-emerald-900/65">
                                ...
                                {prop.payment_method ? (
                                  <>
                                    ...
                                    <span className="capitalize">
                                      {prop.payment_method.replace(/_/g, " ")}
                                    </span>
                                  </>
                                ) : null}
                              </div>
                            </div>
                            <p className="shrink-0 text-base font-semibold tabular-nums text-emerald-950">
                          ...
                          <dl className="mt-3 grid gap-x-6 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
                            <div>
                              <dt className="text-[10px] font-semibold uppercase tracking-widest text-emerald-800/70">
                                Agent
                              </dt>
                              <dd className="mt-0.5 text-emerald-950">{agentLabel ?? "—"}</dd>
                            </div>
                            <div>
                              <dt className="text-[10px] font-semibold uppercase tracking-widest text-emerald-800/70">
                                Agent commission
                              </dt>
                              <dd className="mt-0.5 tabular-nums text-emerald-950">
                          ...
                          <div className="mt-3 flex flex-wrap gap-4">
                            ...
                            >
                              {prop.property_id ? "Inventory" : "Property record"}
              ...
              <div className="overflow-hidden rounded-[14px] border border-indigo-200/80 bg-indigo-50/40">
                <div className="h-0.5 bg-indigo-600" />
                <div className="flex items-center justify-between gap-3 border-b border-indigo-200/70 bg-indigo-100/55 px-5 py-4">
                  <div>
                    <h2 className="text-sm font-semibold text-indigo-950">
                      Deals
                      <span className="ml-2 font-medium text-indigo-800/70">
                        ({deals?.length ?? 0})
                      </span>
                    </h2>
                    <p className="mt-0.5 text-xs text-indigo-900/65">
                      Pipeline opportunities for this client
                    </p>
                  </div>
                ...
                <div className="space-y-2 p-3">
                  {(deals ?? []).length === 0 ? (
                    <p className="px-2 py-4 text-sm text-indigo-900/60">No deals yet.</p>
                  ) : (
                    (deals ?? []).map((deal) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center justify-between rounded-[10px] border px-3 py-2.5 transition-colors hover:brightness-[0.98] ${colors.bg} ${colors.border}`}
                        >
                          <div className="min-w-0">
                            <p className={`text-sm font-semibold ${colors.text}`}>{deal.title}</p>
                            <p className="text-xs capitalize text-muted-foreground">
                              {deal.deal_type.replace(/_/g, " ")}
                              {deal.property_title ? ` · ${deal.property_title}` : ""}
                            </p>
                          </div>
                          <div className="flex shrink-0 items-center gap-3">
                            ...
                            <span
                              className={`inline-flex rounded-full border bg-white/70 px-2.5 py-0.5 text-xs font-medium capitalize ${colors.border} ${colors.text}`}
                            >
                      ...
                      <div key={act.id} className="flex items-start gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatDate(act.created_at, "dd MMM yyyy, HH:mm")}
                          </p>
                        </div>
                      </div>
                    ))
                ...
                <div className="overflow-hidden rounded-[14px] border border-border bg-[#1B2430] p-4 text-[#E8E4DC]">
                  ...
                  <Link href={`/leads/${originatingLead.id}`} className="block hover:underline">
                    ...
                    <p className="mt-1 text-xs capitalize opacity-80">
                      {originatingLead.source.replace(/_/g, " ")} ·{" "}
                      {originatingLead.interest.replace(/_/g, " ")}
                    </p>
                  </Link>
                </div>
              ) : null}
