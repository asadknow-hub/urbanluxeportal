...
const sql = fs.readFileSync(
  path.join("supabase", "migrations", "0018_lead_remaining_picklists.sql"),
  "utf-8"
);
...
try {
  ...
  const check = await client.query(
    "select field_key, count(*)::int as n from public.lead_field_options group by field_key order by field_key"
  );
  console.log("Applied 0018.", check.rows);
} finally {
  await client.end();
}
