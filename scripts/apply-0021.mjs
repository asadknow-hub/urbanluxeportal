...
const sql = fs.readFileSync(
  path.join("supabase", "migrations", "0021_drop_other_department_tables.sql"),
  "utf-8"
);
...
try {
  ...
  const tables = await client.query(`
    ...
    where table_schema = 'public'
      and table_name in (
        'properties','property_owners','property_media',
        'quotations','quotation_items','invoices','invoice_items',
        'payments','cheques','expenses','approvals','campaigns',
        'form_submissions','import_batches','routing_rules','automation_rules',
        'leads','deals','customers','documents','profiles'
      )
    order by table_name
  `);
  console.log(
    "Applied 0021. remaining of interest:",
