...
const sql = fs.readFileSync(
  path.join("supabase", "migrations", "0013_lead_nationality_and_follow_ups.sql"),
  "utf-8"
);
...
try {
  ...
  const nats = await client.query("select count(*)::int as n from public.lead_nationalities");
  const follows = await client.query("select count(*)::int as n from public.lead_follow_ups");
  console.log("Applied 0013. nationalities:", nats.rows[0].n, "follow_ups:", follows.rows[0].n);
} finally {
  await client.end();
}
