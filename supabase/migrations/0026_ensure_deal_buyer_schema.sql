-- Repair: production was missing deal transaction columns (buyer_email and related)
-- so convert-to-deal failed with a PostgREST schema cache error.
-- Safe to re-run. Also reloads the API schema cache.
...
alter table public.customer_properties
  ...
  add column if not exists agent_commission_amount bigint,
...
create index if not exists idx_customer_properties_agent on public.customer_properties (assigned_to);
