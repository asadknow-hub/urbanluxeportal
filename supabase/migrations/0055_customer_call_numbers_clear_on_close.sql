-- Customer dial list (parity with leads.call_numbers) + clear CRM tasks on deal close.

alter table public.customers
  add column if not exists call_numbers text[] not null default '{}';

-- Backfill from originating / linked leads.
update public.customers c
set call_numbers = coalesce(
  (
    select l.call_numbers
    from public.leads l
    where l.id = c.lead_id
      and l.call_numbers is not null
      and cardinality(l.call_numbers) > 0
    limit 1
  ),
  c.call_numbers
)
where c.deleted_at is null
  and cardinality(c.call_numbers) = 0
...
begin
  ...
  if v_deal.property_id is null then
    raise exception 'Confirmed inventory property is required before finalizing';
  end if;
  ...
  v_agency_commission := v_deal.agency_commission_amount;
  if v_agency_commission is null and v_deal.agency_commission_rate is not null and coalesce(v_deal.value, 0) > 0 then
    v_agency_commission := round(v_deal.value * v_deal.agency_commission_rate / 100.0);
  end if;
  ...
  if v_customer_id is null then
    insert into public.customers (
      ...
      call_numbers, assigned_to, lead_id, status, lead_context, client_since, created_by
    )
    values (
      'individual',
      v_name,
      ...
      v_deal.kyc_trn,
      coalesce(case when v_lead.id is not null then v_lead.call_numbers else null end, '{}'),
      v_deal.assigned_to,
      v_deal.lead_id,
      'active',
      v_deal.lead_context,
    ...
    update public.customers set
      ...
      call_numbers = case
        when cardinality(call_numbers) > 0 then call_numbers
        when v_lead.id is not null and v_lead.call_numbers is not null then v_lead.call_numbers
        else call_numbers
      end,
  ...
  insert into public.customer_properties (
    customer_id, deal_id, deal_type, property_id, property_title, property_community,
    property_building, property_unit, property_ref, property_snapshot, property_type,
    value, payment_method, payment_snapshot,
    assigned_to, agent_name, agent_commission_amount, agent_commission_rate,
    agency_commission_amount, agency_commission_rate
  )
  select
    v_customer_id,
    p_deal_id,
  ...
  if v_deal.lead_id is not null then
    ...
    -- Close open CRM tasks for this lead once the deal is won.
    update public.lead_follow_ups set
      status = 'skipped',
      updated_at = now()
    where lead_id = v_deal.lead_id
      and status = 'scheduled';

    update public.lead_viewings set
      status = 'cancelled'
    where status = 'scheduled'
      and (
        deal_id = p_deal_id
        or lead_id = v_deal.lead_id
      );
  else
    update public.lead_viewings set
      status = 'cancelled'
